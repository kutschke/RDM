...
\fixme{Should I use the word ``dataset'' instead of ``file stream''?
  I am using a distinct word for now since the idea of dataset is well defined after the
  data is in SAM.  Will there be properties of files early in the RDM that do not
  map onto that notion of dataset?
}
...
\fixme{We need to address whether data streams or file streams are exclusive or if one
  event may be in several data streams; we also need to address if one data stream may
  be in more than one file stream. }
...
The base design of the TDAQ is that
...
The concept generalizes.  The TDAQ will also write the intensity stream \fixme{reference to define this}.
...
can tell \art that subruns will be atomic by \fixme{say how to do this}.
