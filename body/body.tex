...
\section{Event WindowTags and EventMode}
...
this includes two elemnents that will be discused below.

\fixme{As Ryan what the right name for configuration packet is.}
...
enough bits have been reserved for it to produce unique identifiers for each event.
...
The detector readout electronics label their information with the EWT,
...
The detector readout electronics have pre-programmed operations defined for each mode.
When it creates the configuration packed for each event,
the CFO sets a bit field called the Event Mode (EM);
the detector electronics read these bits
and perform the pre-progammed operations for the given mode.
...
that the event happend.
The EWT will be recorded as part of the \art data payload for each event.
...
happen during the off-spill period to ensure that each on-spill
...
such a range may span runs, but need not.
...
On run boundaries the TDAQ system will do some house keeping,
including resetting some hardware, reloading some firmware and restarting some processes.
These are steps will cure any Single Event Upsets (SEU) in the firmware
and will generally ensure a robust TDAQ system.
...
Each \art trigger filter process send the events that pass the trigger over a standard ethernet
to one of the two data logger nodes where they are received by an {\code artdaq::DataLogger}
process, which writes the events to files in the DLLS.
...
The TDAQ design configuration has two Data Logger nodes, each running an
...
Within a each subrun, however, the events will not be in order.

\fixme{If we end up with short subruns, say 10 MI cycles, 14 s, will we need to stripe
  this 3 or 4 wide to survive long processing latencies?}

The above story is almost exactly the same correct for both on-spill and off-spill running.
