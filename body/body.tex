...
it will also describe the some elements of the downstream data processing as seen
...
The document includes several appendices that discuss details that provide
...
It is assumed that the reader knows has knows the roles played within the Mu2e TDAQ by the following:
\begin{description}
\item[OTS DAQ] ``Off-the-shelf DAQ''~\cite{MU2EOTSDAQ}: the software used to provide configuration and top level run-time control of the TDAQ system.
  \begin{itemize}
  \item OTS DAQ stores configurations in an instance of a MongoDB database.  This is variously called the ``OTS database'' or the ``Mongo database''.
  \end{itemize}
...
It is not necessary to know details of these packages.
...
\begin{enumerate}
\item Does DPC have any responsibilities to move information from EPICS to anywhere?
