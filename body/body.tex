...
This section describes knowledge that it assumes that the reader has.
...
\item Run and subrun data product fragments; aggregation of run and subrun products.
...
\fixme{Get a reference for the pixel telescopye summary data.}
...
\begin{enumerate}
\item Suppose that one event is taking a very long time in one of the trigger filter processes.
  \art does not have a way to interrupt the event and continue with the next event.
  The only options are to let it finish or to kill the process and restart it.
  It is possible to put filter modules in the path and identify events that are likely
  to be dangerous based on some proxy.  A final option would be to instrument some critical
  loops and limit the count.  In both cases the events can be diverted to a separate stream;
  if necessary they can be prescaled.
